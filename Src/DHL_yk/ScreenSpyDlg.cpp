...
void CScreenSpyDlg::OnSize(UINT nType, int cx, int cy) 
{
	...
	if ((rect.right + m_HScrollPos) > m_lpbmi->bmiHeader.biWidth)	
		pInterlockedExchange((PLONG)&m_HScrollPos, m_lpbmi->bmiHeader.biWidth - rect.right);
	
	if ((rect.bottom + m_VScrollPos) > m_lpbmi->bmiHeader.biHeight)
		pInterlockedExchange((PLONG)&m_VScrollPos, m_lpbmi->bmiHeader.biHeight - rect.bottom);
	...
	if (rect.right >= m_lpbmi->bmiHeader.biWidth && rect.bottom >= m_lpbmi->bmiHeader.biHeight)
	{
		...
		pInterlockedExchange((PLONG)&m_HScrollPos, 0);
		pInterlockedExchange((PLONG)&m_VScrollPos, 0);
 		RECT	rectClient, rectWindow;
 		GetWindowRect(&rectWindow);
 		GetClientRect(&rectClient);
 		ClientToScreen(&rectClient);
...
void CScreenSpyDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	InterlockedExchangeT pInterlockedExchange=(InterlockedExchangeT)GetProcAddress(LoadLibrary("KERNEL32.dll"),"InterlockedExchange");
	pInterlockedExchange((PLONG)&m_HScrollPos, i);
...
void CScreenSpyDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	InterlockedExchangeT pInterlockedExchange=(InterlockedExchangeT)GetProcAddress(LoadLibrary("KERNEL32.dll"),"InterlockedExchange");
	pInterlockedExchange((PLONG)&m_VScrollPos, i);
