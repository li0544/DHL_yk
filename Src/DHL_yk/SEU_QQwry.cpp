...
long SEU_QQwry::GetRecordCount(void)//得到总的IP记录数
{
	if(!m_bOpen) 
		return 0;
...
* 说明:
*********************************************************************/
CString SEU_QQwry::GetCountryLocal(long index)
{
/*
	int js=0;
	while(1)
	{
	    long at=GetRecordCount();  //
    ...
    CString str,str1;
	str.Format(_T("抱歉！IP地址数据库出错！:%d"),js); 
	str1=GetCountryLocals(m_ei.bMode,GetStartIPInfo(index)+4);
	str1.Format(_T("%s:%d"),str1,js);
...
*/
	if(index<0 || index>GetRecordCount()-1)
		return "抱歉！IP地址数据库出错！";
	return GetCountryLocals(m_ei.bMode,GetStartIPInfo(index)+4);

}

CString SEU_QQwry::GetCountryLocals(BYTE bMode,int ioffset)
{
	CString buf="";
	if(bMode==1)//X 没有跳
	{
...
void SEU_QQwry::SaveToFile(CString Name)
{
	...
	for(m_i=0;m_i<GetRecordCount();m_i++)
	{
		...
		m_buf.Format("%-15s %-15s %s\r\n",
			str1,str2,GetCountryLocals(m_ei.bMode,ioff+4));
		WriteFile(hFile, m_buf.GetBuffer(0), m_buf.GetLength(), &dwBytesWrite, NULL);
	}
	CloseHandle(hFile);
}
