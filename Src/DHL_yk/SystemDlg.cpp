	...
	: CDialog(CSystemDlg::IDD, pParent)
{
	...
	m_pContext = pContext;
	m_hIcon = LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_SYSKM));
...
void CSystemDlg::ShowInfo()
{
	...
	m_info.InsertItem(0, _T("\\\\系统信息") , 0);
	m_info.InsertItem(1, _T("CPU_速度:") , 2);
	m_info.InsertItem(2, _T("CPU_信息:"), 2);
	m_info.InsertItem(3, _T("系统名称:"), 2);
	m_info.InsertItem(4, _T("当前用户:"), 2);
	m_info.InsertItem(5, _T("屏幕分辨:"), 2);
	m_info.InsertItem(6, _T("") , 2);
	m_info.InsertItem(7, _T("\\\\服务端信息") , 1);
	m_info.InsertItem(8, _T("上线类型:") , 2);
	m_info.InsertItem(9, _T("连接端口:") , 2);
	m_info.InsertItem(10, _T("运行类型:"), 2);
	m_info.InsertItem(11, _T("服务途径:"), 2);
	m_info.InsertItem(12, _T("杀毒软件:"), 2);
	m_info.SetItemText(1, 1, m_SysInfo.szCpuSpeend);
	m_info.SetItemText(2, 1, m_SysInfo.szCpuInfo);
	m_info.SetItemText(3, 1, m_SysInfo.szPcName);
	m_info.SetItemText(4, 1, m_SysInfo.szUserName);
	m_info.SetItemText(5, 1, m_SysInfo.szScrSize);
	m_info.SetItemText(8, 1, m_SysInfo.LineName);
	m_info.SetItemText(9, 1, m_SysInfo.LinePort);
	m_info.SetItemText(10, 1, m_SysInfo.InstallOpen);
	m_info.SetItemText(11, 1, m_SysInfo.Program);
	m_info.SetItemText(12, 1, m_SysInfo.szUserVirus);
...
BOOL CSystemDlg::OnInitDialog() 
{
	...
	m_tab.InsertItem(3, "系统信息");

	m_info.SetExtendedStyle(/*LVS_EX_FLATSB |*/ LVS_EX_FULLROWSELECT);
	m_info.InsertColumn(0, "属性", LVCFMT_LEFT, 100);
	m_info.InsertColumn(1, "内容", LVCFMT_LEFT, 340);
	...
	m_list_process.InsertColumn(2, "程序路径", LVCFMT_LEFT, 290);

	m_list_windows.SetExtendedStyle(/*LVS_EX_FLATSB |*/ LVS_EX_FULLROWSELECT);
	m_list_windows.InsertColumn(0, "PID", LVCFMT_LEFT, 70);
	m_list_windows.InsertColumn(1, "窗口名称", LVCFMT_LEFT, 370);

	m_list_dialupass.SetExtendedStyle(/*LVS_EX_FLATSB |*/ LVS_EX_FULLROWSELECT);
	m_list_dialupass.InsertColumn(0, "名称", LVCFMT_LEFT, 70);
	m_list_dialupass.InsertColumn(1, "用户", LVCFMT_LEFT, 85);
	m_list_dialupass.InsertColumn(2, "密码", LVCFMT_LEFT, 75);
	m_list_dialupass.InsertColumn(3, "电话/主机", LVCFMT_LEFT, 80);
	m_list_dialupass.InsertColumn(5, "设备", LVCFMT_LEFT, 130);
	...
	m_ImgList.Create(16, 16,ILC_COLOR8|ILC_MASK,15,1);
	m_ImgList.Add(AfxGetApp()->LoadIcon(IDI_ICON_NET));	
