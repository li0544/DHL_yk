...
//			pContext->m_WriteBuffer.Write(m_bPacketFlag, sizeof(m_bPacketFlag));
	 ...
	 //	pContext->m_wsaOutBuffer.len = pContext->m_WriteBuffer.GetBufferLen();
		...
		PostQueuedCompletionStatusT pPostQueuedCompletionStatus=(PostQueuedCompletionStatusT)GetProcAddress(LoadLibrary("KERNEL32.dll"),"PostQueuedCompletionStatus");
 		pPostQueuedCompletionStatus(m_hCompletionPort, 0, (DWORD) pContext, &pOverlap->m_ol);
...
bool CIOCPServer::OnClientReading(ClientContext* pContext, DWORD dwIoSize)
{
	...
	try
	{
		...
		if (GetTickCount() - nLastTick >= 1000)
		{
			...
			pInterlockedExchange((LPLONG)&(m_nRecvKbps), nBytes);
			nBytes = 0;
		}
		...
		while (pContext->m_CompressionBuffer.GetBufferLen() > HDR_SIZE)
		{
			...
			MoveMemory(bPacketFlag, pContext->m_CompressionBuffer.GetBuffer(8), sizeof(bPacketFlag));

			if (memcmp(m_bPacketFlag, bPacketFlag, sizeof(m_bPacketFlag)) != 0)
				throw "bad buffer";

			int nSize = 0;
			MoveMemory(&nSize, pContext->m_CompressionBuffer.GetBuffer(0), sizeof(int));
...
void CIOCPServer::CloseCompletionPort()
{
	...
	while (m_nWorkerCnt)
	{
		pPostQueuedCompletionStatus(m_hCompletionPort, 0, (DWORD) NULL, NULL);
		Sleep(100);
	}
